...
import { Brain, Lightbulb, Building2, Pencil, MessageCircle, ArrowRight, X } from 'lucide-react'
...
const features: Feature[] = [
  {
    ...
    detail:
      '就活を始めるとき、多くの人が「自分に何が向いているかわからない」という状態からスタートします。16キャリアタイプ診断は、BIG5・職業志向・価値観をもとにあなたを16タイプに分類し、強み・弱み・向いている職種を明確にします。24問・約5分・ログイン不要で受診でき、SNSからそのまま受験できる設計です。診断後はAIがあなた個人に合わせたレポートを生成するため、同じタイプでも内容が変わります。',
    capabilities: [
      '16タイプへの分類（タイプ別キャッチコピー・強み・おすすめ職種あり）',
      '強み・弱み・向いている職種の可視化',
      'AIによる個別レポート生成（同タイプでも内容が異なる）',
      'X・LINE・Instagramストーリーへのシェア、PDF・画像保存に対応',
      '面接活用アドバイス付きレポートの提供',
    ],
    ctaLabel: '診断してみる',
  },
  ...
  return (
    <section id="features" className="bg-white py-24 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        {/* モバイル: 横スクロールタブ */}
        <div className="md:hidden flex gap-2 overflow-x-auto -mx-6 px-6 pb-2 mb-4">
          {features.map((feature, index) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-full text-sm font-medium shrink-0 transition-all duration-200 whitespace-nowrap"
                style={{
                  backgroundColor: isActive ? '#299dd9' : '#f5f5f5',
        ...
        <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4 items-start">
          ...
          {/* 右: 選択中の機能詳細 */}
          <div
            className="rounded-2xl border p-5 md:p-8 flex flex-col gap-5 min-h-80"
          ...
          >
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
              ...
              {!selected.comingSoon && selected.ctaLabel && (
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 h-10 px-5 rounded text-sm font-medium text-white bg-[#299dd9] hover:bg-[#2490cc] transition-colors duration-200 sm:shrink-0"
                >
                  {selected.ctaLabel}
                  <ArrowRight size={14} />
                </a>
            ...
            {selectedIndex === 0 && (
              <div className="mt-2 relative rounded-xl overflow-hidden border border-[#e5e5e5] h-[220px] md:h-[480px]">
                ...
                />
                <div
                  className="absolute inset-x-0 bottom-0 h-[90px] md:h-[180px]"
                  style={{ background: 'linear-gradient(to bottom, transparent, rgba(255,255,255,0.9) 55%, #ffffff)' }}
                />
      ...
      {modalImage && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 md:px-6 md:py-4 border-b border-[#e5e5e5] shrink-0">
              <p className="text-xs md:text-sm font-bold text-[#1a1a1a]" style={{ fontFamily: 'var(--font-heading)' }}>
                {modalImage.title}
              </p>
              <button
                onClick={() => setModalImage(null)}
